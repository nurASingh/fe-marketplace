...
(define-map beneficiaries ((nft-index uint)) (
    (address1 principal) (share1 uint)
    (address2 principal) (share2 uint)
    (address3 principal) (share3 uint)
    (address4 principal) (share4 uint)
    (address5 principal) (share5 uint)
    (address6 principal) (share6 uint)
    (address7 principal) (share7 uint)
    (address8 principal) (share8 uint)
    (address9 principal) (share9 uint)
    (address10 principal) (share10 uint)
  )
)
...
(define-public (mint-token (asset-hash (buff 32)) (max-editions uint)
    ...
    (let
        ...
        (map-insert beneficiaries ((nft-index mintCounter)) (
                  (address1 address1) (share1 share1)
                  (address2 address2) (share2 share2)
                  (address3 address3) (share3 share3)
                  (address4 address4) (share4 share4)
                  (address5 address5) (share5 share5)
                  (address6 address6) (share6 share6)
                  (address7 address7) (share7 share7)
                  (address8 address8) (share8 share8)
                  (address9 address9) (share9 share9)
                  (address10 address10) (share10 share10)
        ))
...
(define-private (payment-split (nft-index uint))
    (let
        (
            ...
            (address1 (unwrap! (get address1 (map-get? beneficiaries {nft-index: nft-index})) failed-to-mint-err))
            (share1 (unwrap! (get share1 (map-get? beneficiaries {nft-index: nft-index})) failed-to-mint-err))
            (address2 (unwrap! (get address2 (map-get? beneficiaries {nft-index: nft-index})) failed-to-mint-err))
            (share2 (unwrap! (get share2 (map-get? beneficiaries {nft-index: nft-index})) failed-to-mint-err))
            (address3 (unwrap! (get address3 (map-get? beneficiaries {nft-index: nft-index})) failed-to-mint-err))
            (share3 (unwrap! (get share3 (map-get? beneficiaries {nft-index: nft-index})) failed-to-mint-err))
            (address4 (unwrap! (get address4 (map-get? beneficiaries {nft-index: nft-index})) failed-to-mint-err))
            (share4 (unwrap! (get share4 (map-get? beneficiaries {nft-index: nft-index})) failed-to-mint-err))
            (address5 (unwrap! (get address5 (map-get? beneficiaries {nft-index: nft-index})) failed-to-mint-err))
            (share5 (unwrap! (get share5 (map-get? beneficiaries {nft-index: nft-index})) failed-to-mint-err))
            (address6 (unwrap! (get address6 (map-get? beneficiaries {nft-index: nft-index})) failed-to-mint-err))
            (share6 (unwrap! (get share6 (map-get? beneficiaries {nft-index: nft-index})) failed-to-mint-err))
            (address7 (unwrap! (get address7 (map-get? beneficiaries {nft-index: nft-index})) failed-to-mint-err))
            (share7 (unwrap! (get share7 (map-get? beneficiaries {nft-index: nft-index})) failed-to-mint-err))
            (address8 (unwrap! (get address8 (map-get? beneficiaries {nft-index: nft-index})) failed-to-mint-err))
            (share8 (unwrap! (get share8 (map-get? beneficiaries {nft-index: nft-index})) failed-to-mint-err))
            (address9 (unwrap! (get address9 (map-get? beneficiaries {nft-index: nft-index})) failed-to-mint-err))
            (share9 (unwrap! (get share9 (map-get? beneficiaries {nft-index: nft-index})) failed-to-mint-err))
            (address10 (unwrap! (get address10 (map-get? beneficiaries {nft-index: nft-index})) failed-to-mint-err))
            (share10 (unwrap! (get share10 (map-get? beneficiaries {nft-index: nft-index})) failed-to-mint-err))
            (saleType (unwrap! (get sale-type (map-get? sale-data {nft-index: nft-index})) amount-not-set))
        ...
        (if (> share2 u0) (pay-royalty saleAmount address2 share2) (ok true))
        (if (> share3 u0) (pay-royalty saleAmount address3 share3) (ok true))
        (if (> share4 u0) (pay-royalty saleAmount address4 share4) (ok true))
        (if (> share5 u0) (pay-royalty saleAmount address5 share5) (ok true))
        (if (> share6 u0) (pay-royalty saleAmount address6 share6) (ok true))
        (if (> share7 u0) (pay-royalty saleAmount address7 share7) (ok true))
        (if (> share8 u0) (pay-royalty saleAmount address8 share8) (ok true))
        (if (> share9 u0) (pay-royalty saleAmount address9 share9) (ok true))
        (if (> share10 u0) (pay-royalty saleAmount address10 share10) (ok true))
        (ok true)
    )
)

(define-private (pay-royalty (saleAmount uint) (payee principal) (share1 uint))
    (begin
        (stx-transfer? (/ (* saleAmount share1) u10000) tx-sender payee)
        (ok true)
    )
)
